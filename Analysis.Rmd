...
plyr::rbind.fill(lapply(all_contestants, function(x){
  ...
  write.csv("Analysis Files//Contestant_daily_mentions.csv", row.names = F)
...
plyr::rbind.fill(lapply(all_contestants, function(x){
  ...
  write.csv("Analysis Files//total_mentions_contestant.csv")
...
plyr::rbind.fill(lapply(all_contestants, function(x){
  ...
  write.csv("Analysis Files//Controversial_Contst.csv")
...
plyr::rbind.fill(lapply(all_contestants, function(x){
  ...
  write.csv("Analysis Files//Bing_Contest_by_Weekday.csv")
...
plyr::rbind.fill(lapply(all_contestants, function(x){
  ...
  write.csv("Analysis Files//Bing_Contest_by_hr.csc")
```
...
plyr::rbind.fill(lapply(all_contestants, function(x){
  df[grepl(x, tolower(df$tweet)),] %>%
    summarise(handles = users(tweet) %>% 
                ...
                length()) %>%
    mutate(contestant = x)})) %>% 
  write.csv("Analysis Files//Contst_handles_count.csv")
...
plyr::rbind.fill(lapply(all_contestants, function(x){
  ...
  count(UserIsVerified) %>% 
    mutate(name = x)})) %>% 
  write.csv("Analysis Files//Handles_verified_contst.csv")
